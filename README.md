...
| **Phase** | 1 완료 (MVP) |
| **테스트** | 60/60 통과 ✅ |
| **이슈** | 0개 열림 (최근 13개 모두 해결) |
| **PR** | 0개 열림 |
| **다음 단계** | Phase 2: LLM 기반 요약/인사이트 |

**최종 업데이트**: 2026-06-11

### 🔧 Phase 1 완료 후 적용된 13개 버그 수정 (2026-06-11)

**타이밍 & 인코딩**
- (#1) DateTime naive/aware 통일: 모든 타임스탐프를 UTC aware로 변환
- (#2) Windows 콘솔 이모지 인코딩: stdout을 UTF-8로 래핑

**캐시 & 중복 제거 안전성**
- (#3) Dedup 캐시 저장 타이밍: 리포트 저장 성공 후에만 캐시 커밋
- (#5) HackerNews URL 해시 충돌: URL 없는 항목은 "source:title"로 대체 해싱

**파이프라인 강건성**
- (#7) CollectorManager max_workers=0 크래시: 모든 소스 비활성화 시 빈 리스트 반환

**필터링 & 설정**
- (#6) 키워드 부분매칭 오탐: 단어 경계 매칭으로 변경
- (#8) PROCESSING 설정 구현: min_word_count, cache_ttl_hours, exclude_keywords 활성화

**기능**
- (#4) 트렌드 분석 미포함: HTML 리포트에 트렌드 섹션 추가

**위생**
- (#9) 미사용 의존성 제거: requirements.txt 24개 → 15개 (9개 삭제)
- (#10) .gitignore 정리: 부정 패턴 제거, lock 파일 추적 해제
- (#12) 문서 동기화: README 내용을 코드와 일치시킴

**테스트 인프라**
- (#13) Pytest 테스트 추가: 0개 → 60개 (12개 테스트 파일)
