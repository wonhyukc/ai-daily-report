...
class TestSourceWeight:
    def test_arxiv_outweighs_rss(self):
        ranker = Ranker()
        arxiv_score = ranker._calculate_score(make_ranked_dict(source="ArXiv"))
        rss_score = ranker._calculate_score(make_ranked_dict(source="RSS"))
        assert arxiv_score == 2.0
        assert rss_score == 1.0

    def test_unknown_source_defaults_to_weight_one(self):
        ranker = Ranker()
        assert ranker._calculate_score(make_ranked_dict(source="Unknown")) == 1.0


class TestBonuses:
    def test_competitor_mention_adds_bonus(self):
        ranker = Ranker()
        neutral = ranker._calculate_score(make_ranked_dict())
        with_competitor = ranker._calculate_score(
            make_ranked_dict(title="OpenAI launches new product")
        )
    ...
    def test_high_priority_category_adds_bonus(self):
        ...
        with_category = ranker._calculate_score(
            make_ranked_dict(categories=["Security"])
        )
        assert with_category == neutral + 1.0

    def test_recent_item_gets_recency_bonus(self):
        ranker = Ranker()
        old = ranker._calculate_score(make_ranked_dict())
        recent = ranker._calculate_score(
            make_ranked_dict(published_at=datetime.now(timezone.utc))
        )
        assert recent > old
...
class TestRankItems:
    def test_items_sorted_by_score_descending(self):
        ranker = Ranker()
        items = [
            make_ranked_dict(title="low priority item", source="RSS"),
            make_ranked_dict(title="research paper", source="ArXiv"),
        ]
